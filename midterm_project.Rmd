...
```{r}
water_wheel_data = read_excel("water_wheel.xlsx", skip = 8) 
water_wheel_data = janitor::clean_names(water_wheel_data)
water_wheel_data = mutate(water_wheel_data, 
...
water_wheel_data_1 = water_wheel_data %>%
  group_by(sex) %>%
...
Declare of Distribution: There are `r nrows(water_wheel_data)` participants. By looking at the hitogram I ploted, we can see that the distribution of ages are very similar among males and females. The only difference is that the number of females among all ages is greater than the number of males.
...
```{r}
water_wheel_data_2 = water_wheel_data %>%
...
p1 = ggplot(water_wheel_data_2, aes(x = fhp_size_mm,  fill = sex)) + geom_density(alpha = .6) + facet_grid(~age_group)
water_wheel_data_2


water_wheel_data_3_1 = water_wheel_data %>%
  mutate(age_group = factor(age_group, levels = c(1,2,3,4,5,6,7,8), labels = c("1-17", "18-30", "31-40", "41-50", "51-60", "60+", "60+", "60+"))) %>%
  filter(!(age_group == "1-17")) %>%
  group_by(age_group, sex) %>%
  summarise(total_people = n()) 

water_wheel_data_3_2 = water_wheel_data %>%
  mutate(age_group = factor(age_group, levels = c(1,2,3,4,5,6,7,8), labels = c("1-17", "18-30", "31-40", "41-50", "51-60", "60+", "60+", "60+"))) %>%
  filter(!(age_group == "1-17")) %>%
  group_by(age_group, sex) %>%
  filter(eop_size != 0) %>%
...
water_wheel_data_3 = bind_cols(water_wheel_data_3_1, water_wheel_data_3_2) %>%
  mutate(rate = enlarged_eop_people/total_people)

p2 = ggplot(water_wheel_data_3,aes(group = sex, x = age_group, y = rate)) + geom_line(aes(color = sex))
water_wheel_data_3
...
water_wheel_data_4 = water_wheel_data %>%
  mutate(age_group = factor(age_group, levels = c(1,2,3,4,5,6,7,8), labels = c("1-17", "18-30", "31-40", "41-50", "51-60", "60+", "60+", "60+"))) %>%
  filter(!(age_group == "1-17")) %>%
...
```{r}
water_wheel_data_count = water_wheel_data %>%
  mutate(age_group = factor(age_group, levels = c(1,2,3,4,5,6,7,8), labels = c("1-17", "18-30", "31-40", "41-50", "51-60", "60+", "60+", "60+"))) %>%
  group_by(age_group) %>% 
  summarise(number_of_people = n())

water_wheel_data_mean = water_wheel_data %>%
  drop_na() %>%
  filter(age_group != 1) %>%
  group_by(sex) %>%
  select(fhp_size_mm) %>%
  ...
  select(-fhp_size_mm)
water_wheel_data_mean = unique(water_wheel_data_mean, fromLast = TRUE)


water_wheel_data_prevelence = water_wheel_data %>%
  filter(!(eop_size == 0)) %>% 
  filter(! (eop_size == 1)) %>%
  filter(!(age_group == 1))

number_of_diseased_people = nrow(water_wheel_data_prevelence)

total_people = nrow(water_wheel_data)

prevalence = number_of_diseased_people/total_people
prevalence

water_wheel_data_FHP = water_wheel_data %>%
  filter(fhp_size_mm > 40) %>%
  filter(age_group %in% c(6, 7, 8))
number_of_target = nrow(water_wheel_data_FHP)


total_people = water_wheel_data %>%
  filter(age_group %in% c(6,7,8))
